...
このリンクのファイル名が `.pdf` で終わっている場合、プラグインがリンクを含む段落（枠）の下に
**「View PDF」** ボタンを自動的に追加します。押すとボタンの直後に pdf.js ビューアが展開され、以下の操作ができます。
...
初回表示が重くなりにくくなっています。「Close PDF」で描画済みの内容とレンダリングタスクを破棄します。
