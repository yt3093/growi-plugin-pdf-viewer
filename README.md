...
リンクの URL に `#page=N` を付けると、展開時にそのページへ自動的にジャンプします。

```markdown
[report.pdf](/attachment/66dfd1205b6b5996f1b012a7#page=5)
```

「この資料の5ページ目を参照」のような Wiki 内での引用がしやすくなります。この記法は Adobe の PDF 標準的な
記法（PDF Open Parameters）に準じており、CORS フォールバック時のブラウザ標準ビューア（`<iframe>`）でも
同様にそのページが開きます。

### 読み込みの進捗表示と自動リトライ

読み込み中は「読み込み中… 45%」のように進捗率を表示します（サーバーがファイルサイズを返さない環境では
パーセンテージなしの表示になります）。読み込みに失敗した場合、エラー表示をせずバックグラウンドで自動的に
1 回だけ再読み込みを試みます（一時的なネットワーク不調はここで気付かれずに直ることがあります）。それでも
失敗した場合に、簡易表示（iframe フォールバック）へ切り替わります。
...
- GROWI が Amazon S3 / Google Cloud Storage を Redirect Mode で利用している構成（**GROWI Cloud を含む**）
  ...
  - この簡易表示に切り替わる前に、添付ファイルへ実際に到達できるかを内部的に確認しています。ネットワーク
    自体に到達できない場合（ファイルが削除された、URL が壊れている等）は簡易表示も行わず、「PDFを表示
    できませんでした。ダウンロードしてご確認ください。」という案内のみを表示します。
